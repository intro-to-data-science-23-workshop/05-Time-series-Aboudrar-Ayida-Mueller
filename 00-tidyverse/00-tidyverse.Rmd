...
3. Time series require relatively many measurements (long lengths) conversely to longitudinal data (pnael data) which often assume fewer measurements but a large number of individuals. 
...
- The tsibble package is used at the tidy stage to "verify if the raw temporal data is appropriate for doswnstream analytics" ie if the raw time-related data is well-structured enough for more advanced analysis that comes later in the data processing pipelines. So 'tsibble' assesses if the data is in a good format for further analysis. [Tsibble Research Paper](https://pdf.earo.me/tsibble.pdf):

.center[
<div align="center">
<br>
<img src="pics/tsibble_step.png" height=200>
</div>
]
---
...
2. *Key* : Storing multiple time series in a single object. There can be one or more variable identifying the entities being measured within the dataset. Defining a key is especially important when the dataset contains multiple types of entities. It allows to describe the specific characteristics that make each type of entity unique. 
In the example of the paper, the table describes the number of tuberculosis cases for each gender across the countries every year. We should thus understand that the key contains at least columns gender and country. 
To summarize, the tsibble key helps us identify and keep track of each observational unit over time. If we have duplicates of key-index pairs when constructing our tsibble, this will fail.
...
'tsibble' is a new way of organizing time date easier than some older method such as ts. It also us to swtich data from the older 'ts' format to the 'tsibble' format by using the function 'as_tsibble'. 
...
```{r, eval = FALSE}
library(tsibbledata) #tsibble data compiles examples of tsibble data 
